...
export default async function handler(req, res) {
  ...
  const { base, codigo } = req.query;
  const b = BASES[base];
  if (!b)      return res.status(400).json({ error: 'Base inválida. Use: porto, ptx ou pex.' });
  if (!codigo) return res.status(400).json({ error: 'Parâmetro "codigo" obrigatório.' });

  try {
    const token = await login(b);
    const r = await fetch(`${b.url}/tracking/ocorrencias/minuta?codigo=${codigo}`, {
